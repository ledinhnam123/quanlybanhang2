...
public class NotifiCationDAO {
	...
	public NotificationResponse findByIdUserId(int userId) {
		...
		//	lisMes.setMessage(listNoti.getMessage());
			...
			MessageEntity mess = messageRepo.findByIdNotiFiId(listNoti.getId());
			
			if(mess != null) {
				lisMes.setMessage(mess.getMessage());
			}
...
//	user read notify message
	 public ListNotifyResponse getNotificationById(int id, int userId) {
		 ...
		 MessageEntity mess = messageRepo.findByIdNotiFiId(id);
		 if(mess!=null) {
			 
			 res.setMessage(mess.getMessage());
